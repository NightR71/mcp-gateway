...
统一接入并管理多个 MCP Server 的智能体工具网关：上层 LLM Agent 只对接网关一个入口，即可调用背后任意多个 MCP Server 的工具，全程覆盖鉴权、限流、日志、指标。

**功能与架构以 `README.md` 为准**：开发前先读 README 的「核心特性」「架构」「设计决策与踩坑」三节；新增能力沿用现有分层与兼容红线（公开契约只做增量扩展）。
