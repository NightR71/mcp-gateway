...
2. **测试闭环**——AI 每轮改动必须过全量测试和 lint，项目有 371 项自动化测试（2026-09-18 实测，随跑随更）加 CI 兜底，失败信息直接喂回 AI 修正，测试是裁判，不靠肉眼审代码；
...
> 数字口径纪律：测试数用实测口径（371 / 2026-09-18，随跑随更）或下限式，不引用旧文档的 156。
...
| 测试 / 质量 / CI | 371 项测试（真实子进程集成 + 故障注入）+ GitHub Actions | project-mcp-gateway |
